...
import { 
  ...
  CreditCard, Banknote, History, Receipt, X, Printer
} from 'lucide-react';
...
export default function Sales() {
  ...
  const checkout = async () => {
    ...
    try {
      ...
      // Build receipt object with cart items for immediate display
      setReceiptSale({
        ...res.data,
  ...
  return (
    <div className="page" style={{ padding: '28px' }}>
      <div className="topbar" style={{ background: 'transparent', border: 'none', padding: '0 0 24px 0', position: 'static' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 800 }}>Point of Sale Terminal</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '11px' }}>Process customer orders and view recent transactions.</p>
        </div>
      ...
      {view === 'terminal' ? (
        <div className="pos-layout">
          <div>
            <div className="toolbar" style={{ marginBottom: 20 }}>
              <div style={{ position: 'relative', flex: 1 }}>
                ...
                <input className="form-input" placeholder="Search products by name..." value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft: 36 }} />
              </div>
            </div>
          ...
          <div className="card" style={{ display: 'flex', flexDirection: 'column', height: 'fit-content', position: 'sticky', top: 100 }}>
            <div className="card-header"><div className="card-title">Current Order</div> <Receipt size={18} color="var(--text-muted)" /></div>
            
            <div style={{ flex: 1, minHeight: 200, marginBottom: 20 }}>
              {cart.length === 0 ? (
                <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)', fontSize: 13 }}>Cart is empty</div>
              ) : (
                cart.map(item => (
            ...
            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 20 }}>
              ...
              <div className="form-group">
                <label className="form-label" style={{ fontWeight: 600, fontSize: 13, color: 'var(--text-muted)' }}>PAYMENT MODE</label>
        ...
        <div className="card" style={{ padding: 0 }}>
          <div className="table-wrapper">
            <table>
              <thead>
                <tr><th>Sale ID</th><th>Date/Time</th><th>Customer</th><th>Items</th><th>Total</th><th>Payment</th><th>Receipt</th></tr>
              </thead>
              <tbody>
                {salesHistory.map(s => (
                  <tr key={s.id}>
                    ...
                    <td><span className="badge badge-accent" style={{ textTransform: 'capitalize' }}>{s.payment_method}</span></td>
                    <td>
